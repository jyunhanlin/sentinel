...
class TestFullPipelineIntegration:
    ...
    async def test_risk_checker_rejects_bad_proposal(self):
        ...
        high_risk_response = (
            ...
            '{"symbol": "BTC/USDT:USDT", "side": "long",'
            ' "entry": {"type": "market"}, "position_size_risk_pct": 1.5,'
            ' "stop_loss": 97000.0,'  # above price = wrong side
            ' "take_profit": [{"price": 99000.0, "close_pct": 100}],'
            ' "suggested_leverage": 10, "time_horizon": "4h",'
            ' "confidence": 0.75, "invalid_if": [],'
            ' "rationale": "Bad stop loss"}\n```'
        )
        ...
        runner = PipelineRunner(
            data_fetcher=_make_data_fetcher(),
            technical_short_agent=TechnicalAgent(
                client=mock_client, label="short_term", candle_count=50,
            ),
            technical_long_agent=TechnicalAgent(
                client=mock_client, label="long_term", candle_count=30,
            ),
            positioning_agent=PositioningAgent(client=mock_client),
            catalyst_agent=CatalystAgent(client=mock_client),
            correlation_agent=CorrelationAgent(client=mock_client),
            proposer_agent=ProposerAgent(client=mock_client),
            external_data_fetcher=_make_external_data_fetcher(),
            pipeline_repo=MagicMock(),
            llm_call_repo=MagicMock(),
            proposal_repo=MagicMock(),
